...
(async function() {
  ...
  if (homeSp && spelers.length) {
    spelers.slice(0,10).forEach(s => {
      homeSp.appendChild(
        makeListItem(
          s.naam || 'Speler',
          [s.leeftijdscategorie, s.positie, s.land].filter(Boolean).join(' · '),
          s.niveau || ''
        )
      );
    });
  ...
  // Home: events van dit weekend (alle events, premium en niet-premium)
  ...
  if (homeEv && events.length) {
    ...
    if (weekendEvents.length) {
      ...
      homeEv.textContent = 'Er zijn nog geen events voor dit weekend geregistreerd.';
    }
  }
